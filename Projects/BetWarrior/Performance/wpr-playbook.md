...
1. **Estrutura/contagem das etapas:** a Slack List `F0BJ2BVLXBJ` **É legível via `slack_read_file("F0BJ2BVLXBJ")`** (corrigido 28/08 — a List é um arquivo CSV, não canal; `slack_read_channel` dá `channel_not_found`, mas `slack_read_file` retorna o CSV com a coluna **Status** por card). **Contar os cards por Status = a distribuição por etapa**, direto, sem depender de print. **Ordenação das colunas é FIXA toda semana, não perguntar:** Lead · Contato realizado · Contato consolidado · Reunião agendada · Reunião realizada · Negociação · Definição contratual · Liberação do painel · Campanha ativa. Status ausente na List = 0. Fallback (se o read_file falhar): print do "The Dashboard" do Darwin. Ago 01-27: Lead 17 · Contato realizado 2 · Contato consolidado 6 · Reunião agendada 1 · Reunião realizada 3 · Negociação 1 · Definição contratual 3 · Liberação do painel 0 · Campanha ativa 4 = 37.
